...
const { runClaudeCode, parseJsonOutput } = require('./claude');
...
function readAgentFile(projectPath, kind) {
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    try {
      ...
      if (err.code === 'ENOENT' || attempt >= 3) return null;
      busySleep(100 * (attempt + 1));
      continue;
    }
    // Agents sometimes decorate the file with a UTF-8 BOM or ```json fences.
    // parseJsonOutput already tolerates exactly that for CLI stdout — throwing
    // away a real verdict over a code fence would be a needless failed phase.
    return parseJsonOutput(text.replace(/^﻿/, '').replace(/^\s*```(?:json)?/i, '').replace(/```\s*$/, ''));
  }
}
...
// Model resolution: an explicit per-role key (e.g. "builder_model") wins, then
// "default_model", then nothing (the CLI's own default). This is the one lever
// for cost control. Builder and Tester are the highest-volume roles — one call
// per phase attempt — so a cheap model there is where the real money is saved.
// Debugger, Design, and Security are low-volume judgment calls where dropping
// too far can cost MORE, by missing a real vulnerability or causing extra
// retries; dial those back up individually if quality suffers.
function modelFor(config, role) {
  return config[`${role}_model`] || config.default_model || null;
}

function fallbackModelFor(config, role) {
  const raw = config[`${role}_fallback_model`] || config.fallback_model || null;
  if (!raw) return null;
...
async function runAgent({ projectPath, config, onSpawn, prompt, kind, timeoutMs, role }) {
  ...
  const fallbackModel = fallbackModelFor(config, role);
  const res = await runClaudeCode({ prompt, cwd: projectPath, config: agentConfig, onSpawn, model, fallbackModel });
...
function runPlanner({ projectPath, config, onSpawn, brief, projectName, capabilities }) {
  return runAgent({
    projectPath, config, onSpawn, kind: 'plan', role: 'planner',
...
async function runBuilder({ projectPath, config, onSpawn, phase, index, total, context, prompt, capabilities, projectState }) {
  const res = await runAgent({
    projectPath, config, onSpawn, kind: 'report', role: 'builder',
...
async function runBuilderFix({ projectPath, config, onSpawn, fixPrompt }) {
  const res = await runAgent({ projectPath, config, onSpawn, kind: 'report', role: 'builder', prompt: fixPrompt });
  res.report = res.answer;
  return res;
}
...
async function runTester({ projectPath, config, onSpawn, phase, context, prompt, report, builderResult, autoSummary, capabilities, regression }) {
  const res = await runAgent({
    projectPath, config, onSpawn, kind: 'verdict', role: 'tester',
...
async function runDebugger({ projectPath, config, onSpawn, phase, context, prompt, history, capabilities }) {
  const res = await runAgent({
    projectPath, config, onSpawn, kind: 'diagnosis', role: 'debugger',
...
async function runDesigner({ projectPath, config, onSpawn, context, capabilities, round, rounds, previousIssues }) {
  const res = await runAgent({
    projectPath, config, onSpawn, kind: 'design', role: 'design',
...
async function runSecurity({ projectPath, config, onSpawn, context, capabilities, target }) {
  const res = await runAgent({
    projectPath, config, onSpawn, kind: 'security', role: 'security',
...
async function runDeployer({ projectPath, config, onSpawn, projectName, repoName, visibility, context, capabilities, target, fixPrompt }) {
  const res = await runAgent({
    projectPath, config, onSpawn, kind: 'deploy', role: 'deployer',
