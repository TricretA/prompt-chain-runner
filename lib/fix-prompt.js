...
// Agent JSON is only ever as well-formed as the model made it. A field the
// schema says is an array can arrive as a bare string, an object, or null, and
// a TypeError here would kill the entire run from inside a prompt builder.
function asList(value) {
  return (Array.isArray(value) ? value : value == null ? [] : [value])
    .map((v) => String(v && typeof v === 'object' ? JSON.stringify(v) : v).trim())
    .filter(Boolean)
    .join(', ');
}
...
function asFindings(value) {
  return (Array.isArray(value) ? value : value == null ? [] : [value])
    .map((f) => (f && typeof f === 'object' ? f : { what: String(f) }));
}
...
function buildFixPrompt({
  ...
  strategy = 'fix', lessons = '', diagnosis = null, attemptHistory = '', reverted = true,
}) {
...
${reverted
  ...
  : 'Several attempts at this task have failed. There was no known-good commit to roll back to, so the previous attempts\' changes are STILL PRESENT in the working tree — inspect what is there and undo or replace whatever is wrong before you build.'}
...
Solve it a DIFFERENT way this time. Choose the simplest, most conventional implementation that satisfies the task; if the earlier attempts were clever, be boring instead. When done, ${REPORT_INSTRUCTION}`;
  ...
  files to change: ${asList(diagnosis.files_to_change) || '(unspecified)'}\n`
    : '';
...
function buildSecurityFixPrompt({ security, context }) {
  const items = asFindings(security.critical).map((c, i) =>
