...
function treeHash(projectPath) {
  // Outside the repo on purpose: in a git worktree or submodule ".git" is a
  // FILE, so a scratch index under it cannot be created and the anti-cheat
  // check would silently disable itself.
  const scratch = path.join(os.tmpdir(), `pcr-index-${process.pid}-${Date.now()}`);
  try {
