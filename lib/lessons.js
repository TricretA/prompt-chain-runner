...
function signature(errorText) {
  ...
  s = s
    // The orchestrator wraps every failure in the same section framing before
    // it gets here. Left in, that boilerplate is shared by ALL errors and can
    // clear the similarity floor on its own, matching unrelated lessons.
    .replace(/^--- automated check .*? ---$/gm, ' ')
    .replace(/^--- the tester agent rejected the work ---$/gm, ' ')
    .replace(/^\s*tester summary:/gm, ' ')
    .replace(/^\s*(?:evidence|suggested fix):/gm, ' ')
    .replace(/\[\.\.\. first \d+ chars truncated \.\.\.\]/g, ' ')
